...
double coninteg1(sinfo& sl, sinfo& sr, xinfo& xi, pinfo& pi, tree x, tree::tree_p& nx, std::vector<double>& pv, std::vector<double>& muv)
{
   ...
   //Rcout << "syall,nall: " << syall << ", " << nall << endl;
   ...
   //Rcout << "mla,mua: " << mla << ", " << mua << endl;

   double ml=mla;
   double mu=mua;
   conint(ml,mu,nx,x,xi);
   //Rcout << "ml,mu: " << ml << ", " << mu << endl;
   ...
   //Rcout << "in coninteg1, iltc: " << iltc << endl;
   for(size_t i=0;i!=pv.size();i++) {
      pv[i] /= iltc;
   //   Rcout << "in coninteg1, i, pv: " << i << ", " << pv[i] << ", " << muv[i] << endl;
   }
...
double coninteg1(sinfo& sl, sinfo& sr, xinfo& xi, pinfo& pi, tree x, tree::tree_p& nx, std::vector<double>& pv, std::vector<double>& muv, double& lcfac)
{
   ...
   //Rcout << "lMax: " << lMax << endl;
   //for(size_t i=0;i!=pv.size();i++) {
    //  Rcout << "in coninteg1, i, pv, priv,muv: " << i << ", " << pv[i] << ", " << priv[i] << ", "  << muv[i] << endl;
   //}
...
//nxid is the nog node
{
   ...
   for(size_t i=0;i!=pv.size();i++) {
      pv[i] /= illrc;
      //Rcout << "i,pv,mul,mur: " << i << ", " << pv[i] << ", " << mulv[i] << ", " << murv[i] << endl;
   }
   //Rcout << "cnt,sumpr: " << cnt << ", " << sumpr << endl;
   //Rcout << "illrc: " << illrc << endl;
   return illrc;
}
...
bool bd(tree& x, xinfo& xi, dinfo& di, pinfo& pi, rn& gen)
{
   ...
   if(gen.uniform() < PBx) { //do birth or death
      ...
      Rcout << "birth prop: node, v, c: " << nx->nid() << ", " << v << ", " << c << "," << xi[v][c] << endl;
      Rcout << "L,U: " << L << "," << U << endl;
      Rcout << "PBx, PGnx, PGly, PGry, PDy, Pnogy,Pbotx:" <<
         PBx << "," << PGnx << "," << PGly << "," << PGry << "," << PDy <<
         ", " << Pnogy << "," << Pbotx << endl;
      Rcout << "left ss: " << sl.n << ", " << sl.sy << ", " << sl.sy2 << endl;
      Rcout << "right ss: " << sr.n << ", " << sr.sy << ", " << sr.sy2 << endl;
      Rcout << "lill, lilr, lilt: " << lill << ", " << lilr << ", " << lilt << endl;
      Rcout << "alphas: " << alpha1 << ", " << alpha2 << ", " << alpha << endl;
      */
      ...
      /*
      Rcout << "death prop: " << nx->nid() << endl;
      Rcout << "nognds.size(), ni, nx: " << nognds.size() << ", " << ni << ", " << nx << endl;
      Rcout << "depth of nog node: " << dny << endl;
      Rcout << "PGny: " << PGny << endl;
      Rcout << "PGlx: " << PGlx << endl;
      Rcout << "PGrx: " << PGrx << endl;
      Rcout << "PBy: " << PBy << endl;
      Rcout << "Pboty: " << Pboty << endl;
      Rcout << "PDx: " << PDx << endl;
      Rcout << "Pnogx: " << Pnogx << endl;
      Rcout << "left ss: " << sl.n << ", " << sl.sy << ", " << sl.sy2 << endl;
      Rcout << "right ss: " << sr.n << ", " << sr.sy << ", " << sr.sy2 << endl;
      Rcout << "lill, lilr, lilt: " << lill << ", " << lilr << ", " << lilt << endl;
      Rcout << "sigma: " << pi.sigma << endl;
      Rcout << "tau: " << pi.tau << endl;
      Rcout << "alphas: " << alpha1 << ", " << alpha2 << ", " << alpha << endl;
      */
...
bool bdc(tree& x, xinfo& xi, dinfo& di, pinfo& pi, rn& gen)
{
   ...
   if(gen.uniform() < PBx) { //do birth or death
      ...
      if((sl.n>=5) && (sr.n>=5)) { //cludge?
         ...
         //Rcout << "first iltc: " << iltc << endl;
         double iltc = coninteg1(sl,sr,xi,pi,x,nx,pv,muv,lfac1);
         //Rcout << "second iltc: " << iltc*exp(lfac1) << endl;
         //Rcout << "in bdc birth, lfac1: " << lfac1 << endl;
         //iltc *= exp(lfac1);
         ...
         //Rcout << "illrc first: " << illrc << endl;
         //Rcout << "size pv 2: " << pv.size() << endl;
         double illrc = coninteg2(sl,sr,xi,pi,y,nxid,pv,mulv,murv,lfac2);
         //Rcout << "illrc second: " << illrc*exp(lfac2) << endl;
         //illrc *= exp(lfac2);
      ...
      if(gen.uniform() < alpha) {
         ...
         //Rcout << "i, mul, mur: " << i << ", " << mulv[i] << ", " << murv[i] << endl;
      ...
      */
      ...
      if(gen.uniform()<alpha) {
         ii = rdisc(&pv[0],gen);
         //Rcout << "accept death, ii, muv.size(): " << ii << ", " << pv.size() << endl;
         //do death
         x.deathp(nx,muv[ii]);
         return true;
      } else {
