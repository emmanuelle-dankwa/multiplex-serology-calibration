...
```{r}
s <- readRDS(file = here::here("data", "dummy-data", "training-data.RDS"))
...
# saveRDS(folds10, file = here::here("data", "dummy-data", "folds10.RDS"))
```


```{r load-data-2}

#folds10 <- readRDS(file = here::here("data", "dummy-data", "folds10.RDS")) # Cross-validation folds (see chunk `define-fold` above)
```
...
wrap.log <- function(folds=folds10, df= s, k=10) {
  ...
  for (i in 1:k){
    ...
    dfl$Brier_scores[i] <- DescTools::BrierScore(x = as.numeric(as.character(test[, 1])), pred = predicted.values)
...
wrap.bart <- function(folds=folds10, df=s, k= 10) {
  ...
  for (i in 1:k){
    ...
    dfl$Brier_scores[i] <- BrierScore(x = as.numeric(as.character(test[, 1])), pred = 1 - predicted.values)
...
wrap.rf <- function(folds=folds10, df= s, k=10){
  ...
  for (i in 1:k){
    ...
    dfl$Brier_scores[i] <- BrierScore(x = as.numeric(as.character(test[, 1])), pred =predicted.values[,2])
...
# Complete test set 
external <- readRDS(file = here::here("data", "dummy-data", "test-data.RDS"))
...
brier_score <- DescTools::BrierScore(x = as.numeric(as.character(external[, 1])), pred = 1 - predicted.values.bm)
...
brier_score_rf <- DescTools::BrierScore(x = as.numeric(as.character(external[, 1])), pred =predicted.values.rf[,2])
...
brier_score_enet <- DescTools::BrierScore(x = as.numeric(as.character(external[, 1])), pred = predicted.values.enet)
...
brier_score_lr <- DescTools::BrierScore(x = as.numeric(as.character(external[, 1])), pred =predicted.values.lr)
...
brier_score_mb  <- DescTools::BrierScore(x = as.numeric(as.character(external[, 1])), pred =  predicted.values.mbart)
